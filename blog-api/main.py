...
async def index(request: Request):
    ...
    return templates.TemplateResponse(
        # @formatter:off
        "index.html",
        {"request": request, "posts": posts_with_authors, "users": users}
        # @formatter:on
    )
...
async def create_post_form(request: Request):
    users = storage.list_users()
    return templates.TemplateResponse(
        "create_post.html", {"request": request, "users": users}
    )
...
async def create_post_form_submit(
    ...
    print(
        f"Received data: title={title}, "
        f"content={content}, author_id={author_id}"
    )
    try:
        ...
        raise (
            HTTPException(
                status_code=400, detail="author_id must be an integer"
            )
        )

    print(f"Converted author_id: {author_id_int}")
    if not author_id_int:
...
async def list_users(request: Request):
    users = storage.list_users()
    return templates.TemplateResponse(
        "list_users.html", {"request": request, "users": users}
    )
...
async def view_user(request: Request, user_id: int):
    ...
    return templates.TemplateResponse(
        "view_user.html",
        # formatter:off
        {"request": request, "user": user, "posts": user_posts}
        # formatter:on
    )
...
async def view_post(request: Request, post_id: int):
    ...
    return templates.TemplateResponse(
        "view_post.html", {"request": request, "post": post, "author": author}
    )
